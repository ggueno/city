...
void translate(const float * __restrict__ matA, const float * __restrict__ vecP, float * __restrict__ matB)
{
	...
	mat4fCMSetCol(matTmp, vecP, 3);
	mat4fMul(matTmp, matA, matB);
}
			...
			float * __restrict__  matA)
{
	...
	mat[0] = side[0];	  mat[4] = side[1];		mat[8] = side[2];	   mat[12] = 0.0f;
	mat[1] = up[0];		  mat[5] = up[1];		mat[9] = up[2];		   mat[13] = 0.0f;
	mat[2] = -forward[0]; mat[6] = -forward[1]; mat[10] = -forward[2]; mat[14] = 0.0f;
